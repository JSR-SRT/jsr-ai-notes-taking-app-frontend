  ...
  return (
    <div className="min-h-screen  bg-stone-300 ">
      <div className="min-h-screen max-w-5xl mx-auto px-6 py-10 bg-stone-300 ">
        <h1 className="text-4xl font-extrabold text-black mb-6 border-4 border-black rounded-lg bg-neutral-400 py-4  text-center">
        ...
        <h2 className="text-2xl font-extrabold text-black mb-6 border-4 border-black rounded-lg bg-neutral-400 py-2 px-2 text-center">
          Public Notes
        </h2>
        {notes.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {notes.map((note) => (
                <div
                  key={note._id}
                  className="bg-neutral-200 border-4 border-black rounded-2xl p-6 shadow-[6px_6px_0_0_#000] hover:shadow-[10px_10px_0_0_#000] transition-all duration-200"
                >
                  <h3 className="text-2xl font-extrabold mb-3 text-black border-2 border-black rounded-lg bg-red-100 px-2 py-1 shadow-[2px_2px_0_0_#000] truncate">
                    {note.title}
                  </h3>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {note.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="bg-zinc-200 border-2 border-black text-black text-xs font-bold px-3 py-1 rounded-full shadow-[1px_1px_0_0_#000] font-mono"
                      >
                        #{tag}
                      </span>
                    ))}
